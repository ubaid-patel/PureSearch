...
export default function SearchBar({ query, setQuery, onSearch, loading }) {
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      try {
        ...
        // 🔥 Catch the 403 and use server data
  ...
  const handleKeyDown = (e) => {
    if (!showDropdown || (liveSuggestions.length === 0 && !isBlocked)) return;
    if (e.key === "ArrowDown") {
      e.preventDefault();
    ...
    <div ref={wrapperRef} className="relative w-full group z-50 mb-10">
      ...
      <form onSubmit={handleSubmit} className="relative w-full">
        ...
        <input
          ...
          onFocus={() => (liveSuggestions.length > 0 || isTyping || isBlocked) && setShowDropdown(true)}
