...
export const ENV = import.meta.env.VITE_MODE || "live"; // "test" for mock data, "live" for real API
...
export default function App() {
  ...
  // ⚙️ 3. HYBRID SORTING: Top 5 Sorted + The Rest Organic
  const sortedResults = useMemo(() => {
    ...
    // STAGE 1: Split the results into the "Top 5 Priority" pile and "Organic" pile
    results.forEach((item, originalIndex) => {
      const primaryEngine = (item.engines && item.engines.length > 0) 
        ? item.engines[0].toLowerCase() 
        : "unknown";

      if (!engineCounts[primaryEngine]) {
        engineCounts[primaryEngine] = 0;
      }

      // First 5 of any engine go to priority sorting
      if (engineCounts[primaryEngine] < 5) {
        engineCounts[primaryEngine]++;
        priorityGroup.push({ item, originalIndex });
      } else {
        // 6th and beyond go straight to the organic pile (keeping their original index)
        organicGroup.push({ item, originalIndex });
      }
    });

    // STAGE 2: Score ONLY the Priority Group
    ...
    priorityGroup.sort((a, b) => {
      ...
      // Group by Engine Priority
      if (scoreA !== scoreB) {
        return scoreA - scoreB; 
      }
      
      // Tie-breaker: Maintain strict original API ranking within the same engine
      return a.originalIndex - b.originalIndex;
    });

    // STAGE 3: Merge them back together! 
    return [
  ...
  const handleSearch = async (q) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 403) {
        // 🔥 Extracting directly from server response
  ...
  return (
    <div className={`h-screen w-full overflow-hidden transition-colors duration-500 ${activeFont} bg-zinc-50 dark:bg-[#050505] text-zinc-900 dark:text-zinc-100 flex flex-col items-center relative selection:bg-indigo-500/30`}>
      ...
      <header className="flex-none w-full max-w-5xl p-6 flex justify-between items-center z-40 relative">
        <button onClick={goHome} className={`${glassStyle} px-4 py-2.5 rounded-xl flex items-center gap-2.5 text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white transition-all hover:scale-105 active:scale-95 cursor-pointer`}>
          <Command size={16} />
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar w-full flex flex-col items-center z-10 relative pb-10">
        <div className={`w-full max-w-3xl px-4 transition-all duration-700 ease-in-out ${results.length === 0 && !loading ? "mt-[10vh]" : "mt-4"}`}>
          {!loading && results.length === 0 && (
            <div className="flex flex-col items-center text-center mb-10 animate-in fade-in zoom-in duration-700">
          ...
          <div className="w-full relative z-30 mb-800">
            <SearchBar query={searchText} setQuery={setSearchText} onSearch={handleSearch} loading={loading} />
          </div>
          ...
          <Results results={sortedResults} loading={loading} />
        </div>
      </main>
    </div>
  );
